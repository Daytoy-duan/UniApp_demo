			...
			<swiper class="swiper" circular :indicator-dots="false" :autoplay="true" :interval="5000"
				...
				<swiper-item v-for="(item, index) in swiperAdList" :key="index">
					<navigator open-type="navigate" :url="'/pages/webview/webview?url='+ encodeURI(item.data.link)">
						<image class="banner-swiper-img" :src="item.data.image" mode="aspectFill"></image>
					</navigator>
				</swiper-item>
			</swiper>
			<!-- 遮罩使用弧形框 -->
...
<script>
import { getLunbo } from '@/common/http.api.js'
	export default {
		data() {
			return {
				// 轮播图数据
		...
		methods: {
			...
			async getAdverts() {
				let adverts = await getLunbo()
				this.swiperAdList = adverts
				console.log(this.swiperAdList)
			}
		}
	}
</script>
